...
use {
    crate::{logger::Logger, playdate_renderer::PlaydateRenderer},
    ...
    bouquet_core::{
        ...
        rendering::{BouquetFloat, BouquetInteger},
    },
    crankstart::{
        crankstart_game,
...
struct PlaydateProgram {
    ...
    speed: ScreenPoint,
    message_bus: MessageBus<ProgramState, Message>,
...
impl PlaydateProgram {
    pub fn new(_playdate: &Playdate) -> Result<Box<Self>, Error> {
        ...
        result.message_bus.send(
            Arc::new(Message::DebugLog(HELLO_MESSAGE.into())),
            &mut result.program_state,
        );
        result
            .message_bus
            .send(Arc::new(Message::Initialize), &mut result.program_state);
        Ok(Box::new(result))
    }
    ...
    fn render(&mut self) -> Result<(), Error> {
        ...
        PlaydateRenderer::render_box(96, 192, 1, point2(INITIAL_X, INITIAL_Y))?;
        PlaydateRenderer::render_box(
            32,
            20,
            1,
            point2(self.location.x + 32, self.location.y + 16),
        )?;
        PlaydateRenderer::render_box(
            32,
            24,
            1,
            point2(self.location.x + 16, self.location.y + 24),
        )?;
        PlaydateRenderer::render_box(32, 28, 1, point2(self.location.x + 0, self.location.y + 32))?;
        PlaydateRenderer::render_box(32, 32, 1, point2(self.location.x + 0, self.location.y + 0))?;
        PlaydateRenderer::render_box(32, 32, 1, point2(self.location.x - 16, self.location.y + 8))?;
        PlaydateRenderer::render_box(
            32,
            32,
            1,
        ...
        PlaydateRenderer::render_box(32, 32, 1, point2(self.location.x + 0, self.location.y + 16))?;
        PlaydateRenderer::render_box(32, 32, 1, point2(self.location.x + 0, self.location.y - 16))?;
        PlaydateRenderer::render_box(96, 96, 1, point2(64, 64))?;
        self.render_message()?;
        Ok(())
    }
}
...
impl Game for PlaydateProgram {
    fn update(&mut self, _playdate: &mut Playdate) -> Result<(), Error> {
        let delta_t: BouquetInteger = 1;
        let ps = &mut self.program_state;
        self.message_bus
            .send(Arc::new(Message::Update(delta_t as BouquetFloat)), ps);
